...
— nothing on screen is a saved constant. This doc covers how to record it.
...
- **Linux** — OBS Studio (free, not preinstalled but one package/flatpak), or just use
  any meeting tool's (Zoom, Meet, Teams) share-screen-and-record — no separate install if
...
"5-minute video" — use this only as a supplement (e.g. embedded in the README) or if
...
— the cast alone has no audio track to carry the talking points below.
...
| 3 | The original Python test suite, live, against the Rust port | ~45s | The unmodified upstream suite runs through a pure-Python shim that forwards every call to the Rust binary over a pipe — Python never runs Rust logic, it's calling it as a subprocess. |
...
| 7 | Performance | ~45s | Headline speedups (x8–x185 depending on workload) — mention the methodology caveats briefly, don't oversell single-machine numbers. |
...
Python would've rounded away instead pushed `get_prev` a full day into the past — same
...
**Second beat if you have time — DECISIONS.md §19.** Differential fuzzing turned up that
